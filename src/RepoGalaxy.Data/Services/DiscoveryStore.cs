...
public sealed class DiscoveryStore
{
    ...
    private static Repository MapRepository(RepositoryEntity x) => new() { Id = x.Id, GitHubId = x.GitHubId, Owner = x.Owner, Name = x.Name, Description = x.Description ?? string.Empty, PrimaryLanguage = x.PrimaryLanguage ?? string.Empty, HtmlUrl = x.HtmlUrl ?? string.Empty, OwnerAvatarUrl = x.OwnerAvatarUrl ?? string.Empty, IsPrivate = x.IsPrivate, IsArchived = x.IsArchived, Stars = x.Stars, Forks = x.Forks, Watchers = x.Watchers, OpenIssues = x.OpenIssues, CreatedAt = x.CreatedAt, UpdatedAt = x.UpdatedAt, LastPushedAt = x.LastPushedAt, DiscoveryScore = x.DiscoveryScore, Topics = Read(x.TopicsJson), CachedAt = x.CachedAt };
